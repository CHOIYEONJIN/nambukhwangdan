package com.example.nambukhwangdan.navigation


object Routes {
    const val OnboardingIntro = "onboarding_intro"
    const val OnboardingNickname = "onboarding_nickname"
    const val Login = "login"

    const val MainHost = "main_host"

    const val Home = "home"
    const val Journal = "journal"
    const val Inbox = "inbox"
    const val Calendar = "calendar"
    const val Settings = "settings"

    const val DiaryWrite = "diary_write"

    const val AnalyzeResult = "analyze_result"
    const val LetterToTomorrow = "letter_to_tomorrow"
    const val NewLetter="NewLetter"

    const val ReplyScreen="ReplyScreen"
}